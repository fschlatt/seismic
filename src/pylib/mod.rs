...
impl PySeismicIndex {
    ...
    pub fn build(
        ...
        let config = Configuration::default()
            ...
            .summarization_strategy(SummarizationStrategy::EnergyPerserving { summary_energy });
        println!("\nBuilding the index...");
        println!("{:?}", config);
    ...
    pub fn batch_search<'py>(
        ...
        queries
            ...
            .map(|query| {
                self.inverted_index
                    .search(query.0, query.1, k, query_cut, heap_factor)
            })
            .collect::<Vec<_>>()
    }
}
...
#[pyclass]
pub struct PySeismicIndexLargeVocabulary {
    inverted_index: InvertedIndex<u32, f16>,
}

#[pymethods]
impl PySeismicIndexLargeVocabulary {
    #[staticmethod]
    pub fn load(index_path: &str) -> PyResult<PySeismicIndexLargeVocabulary> {
        let serialized: Vec<u8> = fs::read(index_path).unwrap();
        let inverted_index = bincode::deserialize::<InvertedIndex<u32, f16>>(&serialized).unwrap();
        Ok(PySeismicIndexLargeVocabulary { inverted_index })
    }
    ...
    pub fn build(
        ...
        min_cluster_size: usize,
        summary_energy: f32,
    ) -> PyResult<PySeismicIndexLargeVocabulary> {
        let dataset = SparseDataset::<u32, f32>::read_bin_file(input_file)
            .unwrap()
            .quantize_f16();
    ...
    pub fn batch_search<'py>(
        ...
        let queries = SparseDataset::<u32, f32>::read_bin_file(query_path).unwrap();
